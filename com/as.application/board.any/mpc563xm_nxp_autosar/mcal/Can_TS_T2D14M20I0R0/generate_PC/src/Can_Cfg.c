...
CONST(Can_MBConfigObjectType, CAN_CONST) MessageBufferConfigs_PC[CAN_MAXMBCOUNT_0] =
    {
        [!VAR "CanControllerIdValue"="0"!][!VAR "ObjectID"="0"!][!VAR "END_COMMA"="0"!][!VAR "MASK_COUNT"="num:i(count(CanConfigSet/*[1]/CanHardwareObject/*))"!][!LOOP "CanConfigSet/*[1]/CanHardwareObject/*"!][!VAR "END_COMMA"="$END_COMMA + 1"!]
        [!NOCODE!]
        /*
...
/*==================================================================================================*/
[!VAR "LoopVar"="0"!]
CONST(Can_ControllerConfigType, CAN_CONST) ControllerConfigs_PC[CAN_MAXCONTROLLERCOUNT_0] = {[!NOCODE!][!VAR "MASK_COUNT2"="num:i(count(CanConfigSet/*[1]/CanController/*))"!][!VAR "END_COMMA2"="0"!][!ENDNOCODE!][!FOR "x" = "0" TO "num:i(count(CanConfigSet/*[1]/CanController/*))"!][!LOOP "CanConfigSet/*[1]/CanController/*"!][!VAR "Loop"="num:i($LoopVar)"!][!IF "CanControllerId = $Loop"!][!VAR "END_COMMA2" = "$END_COMMA2 + 1"!]
[!NOCODE!]
        [!/*
        Sync-seg = is the segment that is used to synchronise the nodes on the bus. A bit edge (if there is a data change) is expected during this segment.
        Prop-Seg = is a period of time that is used to compensate for physical delay times within the network.
    ...
    [!IF "($Prescaler < 1) or ($Prescaler > 256)"!]
        [!ERROR!]
            ...
            Prescaler = CANClockFrequency * (CanControllerTimeQuanta / 1000000000). CANClockFrequency=[Hz]. CanControllerTimeQuanta=[ns].
        [!ENDERROR!]
    [!ENDIF!]
...
[!IF "CanBccSupport = 'true'"!]#if (CAN_BCC_SUPPORT_ENABLE == STD_ON)
    ...
    [!ENDIF!] [!ENDIF!] [!ENDLOOP!]  [!VAR "LoopVar" = "$LoopVar + 1"!][!ENDFOR!]
  };
...
[!IF "ecu:get('Can.CanConfig.ExternalWakeupSupport')='STD_ON'"!]
            [!VAR "ext_wak" = "1"!]
[!ENDIF!][!ENDNOCODE!]
[!SELECT "CanConfigSet/*[1]"!]CONST(Can_ControlerDescriptorType, CAN_CONST) ControlerDescriptors_PC[CAN_MAXCONTROLLERCOUNT_0] =
    {
...
CONST(Can_ConfigType, CAN_CONST) Can_ConfigSet_PC =
    {
        ...
        /* MessageBufferConfigContainer */
        {
            ...
            (Can_HWObjectCountType)[!"num:i(count(CanConfigSet/*[1]/CanHardwareObject/*))"!]U,
        },
