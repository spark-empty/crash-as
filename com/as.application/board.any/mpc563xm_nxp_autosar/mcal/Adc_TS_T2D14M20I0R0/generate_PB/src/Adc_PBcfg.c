...
[!LOOP "AdcConfigSet/*"!][!//
[!VAR "CSET_NR" = "@index+1"!][!//
/** @brief total number of groups in Config [!"num:i($CSET_NR)"!] */
...
extern VAR(Adc_ValueGroupType, AUTOMATIC) [!"AdcResultBufferPointer"!][ADC_CFGSET[!"num:i($CSET_NR)"!]_GROUP_[!"AdcGroupId"!]_CHANNELS];
[!ELSE!][!//
extern VAR(Adc_ValueGroupType, AUTOMATIC) [!"AdcResultBufferPointer"!][ADC_CFGSET[!"num:i($CSET_NR)"!]_GROUP_[!"AdcGroupId"!]_CHANNELS][[!"AdcStreamingNumSamples"!]];
[!ENDIF!][!//
[!ENDLOOP!][!//
[!ENDLOOP!][!//
...
[!ENDLOOP!][!//
[!FOR "loop" = "0" TO "num:i($maxGrp)-1"!]
  NULL_PTR[!IF "$loop<num:i($maxGrp)-1"!],[!ELSE!][!ENDIF!][!//
[!ENDFOR!]
};
...
[!IF "../../AdcHwUnitId = '0'"!][!//
/* Group Assignments ADC0 - Configuration [!"num:i($CSET_NR)"!] */
...
[!LOOP "AdcConfigSet/*"!][!//
[!VAR "CSET_NR" = "@index+1"!][!//
[!VAR "MAX_COUNT"="num:i(count(AdcHwUnit/*))"!][!//
/** @brief definition of Hw units Configuration [!"num:i($CSET_NR)"!] */
STATIC CONST(Adc_LLD_HWUnitConfiguration, ADC_CONST) Adc_HWUnit_CfgPB_[!"num:i($CSET_NR)"!][[!"$MAX_COUNT"!]] =
{
...
[!VAR "MAX_COUNT"="num:i(count(AdcHwUnit/*[AdcHwUnitId=0]/AdcChannel/*))"!][!//
/** @brief definition of channels ADC0 - Configuration [!"num:i($CSET_NR)"!] */
STATIC CONST(Adc_LLD_ChannelConfiguration, ADC_CONST) Adc_Channels_CfgPB_0_[!"num:i($CSET_NR)"!][[!"$MAX_COUNT"!]] = /* array of conversion command messages */
{
[!VAR "MAX_COUNT"="count(AdcHwUnit/*[AdcHwUnitId=0]/AdcChannel/*)-1"!][!//
[!VAR "CRT_COUNT"="0"!][!//
...
[!VAR "MAX_COUNT"="num:i(count(AdcHwUnit/*[AdcHwUnitId=1]/AdcChannel/*))"!][!//
/** @brief definition of channels ADC1 - Configuration [!"num:i($CSET_NR)"!] */
STATIC CONST(Adc_LLD_ChannelConfiguration, ADC_CONST) Adc_Channels_CfgPB_1_[!"num:i($CSET_NR)"!][[!"$MAX_COUNT"!]] = /* array of conversion command messages */
{
[!VAR "MAX_COUNT"="count(AdcHwUnit/*[AdcHwUnitId=1]/AdcChannel/*)-1"!][!//
[!VAR "CRT_COUNT"="0"!][!//
[!LOOP "AdcHwUnit/*/AdcChannel/*"!][!//
[!IF "../../AdcHwUnitId = '1'"!][!//
  /* Hardware Unit [!"../../AdcHwUnitId"!]  --  [!"AdcHwChannel"!] */
...
/** @brief definition of ch's pointer for configuration [!"num:i($CSET_NR)"!] */
STATIC P2CONST(Adc_LLD_ChannelConfiguration, AUTOMATIC, ADC_APPL_DATA) Adc_Channels_CfgPB_[!"num:i($CSET_NR)"!][ADC_MAX_HW_UNITS] =
{
[!IF "AdcHwUnit/*/AdcHwUnitId = '0'"!][!//
  Adc_Channels_CfgPB_0_[!"num:i($CSET_NR)"!],
[!ELSE!][!//
  NULL_PTR,
[!ENDIF!][!//
[!IF "AdcHwUnit/*/AdcHwUnitId = '1'"!][!//
...
STATIC CONST(Adc_LLD_GroupConfiguration, ADC_CONST) Adc_Groups_CfgPB_[!"num:i($CSET_NR)"!][ADC_GROUPS_[!"num:i($CSET_NR)"!]] =
{
...
[!IF "AdcGroupId = $Loop"!][!//
  /* Group[!"AdcGroupId"!] -- Hardware Unit [!"../../AdcHwUnitId"!] */
  {
    ...
    [!IF "../../AdcHwUnitId = '0' "!]ADC_Group[!"AdcGroupId"!]_Assignment_PB_0_[!"num:i($CSET_NR)"!][!ELSE!]ADC_Group[!"AdcGroupId"!]_Assignment_PB_1_[!"num:i($CSET_NR)"!][!ENDIF!],
...
/* Configuration structure [!"num:i($CSET_NR)"!] */
CONST(Adc_ConfigType, ADC_CONST) [!"@name"!] =
{
    Adc_HWUnit_CfgPB_[!"num:i($CSET_NR)"!], /* pointer to Hw unit configurations */
    Adc_Groups_CfgPB_[!"num:i($CSET_NR)"!], /* pointer to group configurations */
    Adc_Channels_CfgPB_[!"num:i($CSET_NR)"!], /* pointer to channel configurations */
    (Adc_GroupType)ADC_GROUPS_[!"num:i($CSET_NR)"!], /* total number of groups */
    #if(ADC_HW_TRIGGER_API == STD_ON)
      (uint8)[!"num:inttohex(AdcDigitalFilterLength)"!], /* low 8 bits of EQADC_ETDFR, minimum system clock count to recognize a hw trigger is 2^[!"num:i(AdcDigitalFilterLength)"!]+1 */
    #endif
        /* Miscellaneous configuration parameters. - Adc_LLD_MultiConfig */
    {
  [!IF "AdcGeneric/AdcTransferType = 'ADC_DMA'"!][!//
    [!"AdcGeneric/AdcTransferType"!], /* DMA driven mode.  */ [!//
  [!ELSE!][!//
    ADC_INTERRUPT, /* Interrupt driven Mode */
  [!ENDIF!][!//
    [!"num:i(count(AdcHwUnit/*/AdcGroup/*))"!]U, /* the number of the maximum groups in the current configuration */
      [!"AdcGeneric/AdcPriorityQueueMaxDepth"!]U, /* depth of the queue */ [!//
  ...
  [!ENDLOOP!]
    ...
    {
      [!IF "AdcHwUnit/*/AdcHwUnitId = '0'"!](uint8)STD_ON[!ELSE!](uint8)STD_OFF[!ENDIF!], /* ADC_HW_UNIT_0 - ON/OFF */
      [!IF "AdcHwUnit/*/AdcHwUnitId = '1'"!](uint8)STD_ON[!ELSE!](uint8)STD_OFF[!ENDIF!] /* ADC_HW_UNIT_1 - ON/OFF */
    }
    }
};
