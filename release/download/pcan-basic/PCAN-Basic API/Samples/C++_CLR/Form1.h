...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->cbbParameter->Items->AddRange(gcnew cli::array< System::Object^  >(21) {L"USB\'s Device Number", L"USB/PC-Card\'s 5V Power", 
				L"Auto-reset on BUS-OFF", L"CAN Listen-Only", L"Debug\'s Log", L"Receive Status", L"CAN Controller Number", L"Trace File", L"Channel Identification (USB)", 
	...
	private: System::Void btnHwRefresh_Click(System::Object^  sender, System::EventArgs^  e) 
	{
		...
		try
		{
			for (int i = 0; i < m_HandlesArray->Length; i++)
			{
				...
				if (safe_cast<UInt16>(m_HandlesArray[i]) <= PCANBasic::PCAN_DNGBUS1)
				...
				else
				{
					...
					//
					stsResult = PCANBasic::GetValue(safe_cast<UInt16>(m_HandlesArray[i]), TPCANParameter::PCAN_CHANNEL_CONDITION, iBuffer, sizeof(UInt32));
					if (((*stsResult) == TPCANStatus::PCAN_ERROR_OK) && ((iBuffer & PCANBasic::PCAN_CHANNEL_AVAILABLE) == PCANBasic::PCAN_CHANNEL_AVAILABLE))
					{
						stsResult = PCANBasic::GetValue(safe_cast<UInt16>(m_HandlesArray[i]), TPCANParameter::PCAN_CHANNEL_FEATURES, iBuffer, sizeof(UInt32));
